...
while ( $the_query->have_posts() ) {
          	$the_query->the_post();
	    $image_attributes = wp_get_attachment_image_src( get_post_thumbnail_id($the_query->post->ID), 'top-thumb' );
...
// 		$output_latest_news_block[$i] = '<label><a href="'.get_the_permalink($latest_news->post->ID).'" class="home_latest_news">'.get_the_title().'</a></label>';
        ...
        $image_attributes = wp_get_attachment_image_src( get_post_thumbnail_id($latest_news->post->ID), 'video-thumb' );
            ...
            <div class="col-12 col-md-9 col-lg-9">
                <div>
                    <p class="post-tagline">
                        <span><a href="'.home_url('/szerzo/').'?a='.$custom_author.'" class="athr_lnk tag-slug">'.$custom_author.' </a></span>
                        <span>|</span>
			 ...
			 if(empty($type)){
				 ...
				 $output.='<img src="'.$image_attributes[0].'"><a href="'.get_the_permalink($the_query->post->ID).'" class="btn'.$type.'">Link</a>';
				  ...
				  $output.='</div><div class="nc_imgContent">
                    <span>
                    <a href="'.home_url('/szerzo/').'?a='.$custom_author.'" class="athr_lnk">'.$custom_author.' </a></span>
					<span>|</span>
					<span>' . get_post_time('H:i') .  '</span>
...
while ( $the_query->have_posts() ) {
  ...
  $image_attributes = wp_get_attachment_image_src( get_post_thumbnail_id($the_query->post->ID), 'releted-thumb' );
  $custom_author = get_field('news_author',$the_query->post->ID);
    ...
    //     $output .= '
    //       <div class="col-12 col-md-5 col-lg-5">we</div>
    //     ';
...
function section5_shortcode( $atts, $content ) {
  ...
  if( $the_query->have_posts() ){
    ...
    $output.='<section class="aguse_mainSection auto_float"><div class="aguse_container auto_float"><div class="row vertical-align">';
  while ( $the_query->have_posts() ) {
          $the_query->the_post();
          ...
          if($i==1){
            ...
            $image_attributes = wp_get_attachment_image_src( get_post_thumbnail_id($the_query->post->ID), 'normalnews-thumb' );
            ...
            //}
              ...
              $output.='<div class="col-lg-4 col-md-4 col-sm-12">
                       <div class="agu_boxCol col_boxA1">
                         <span class="aug_img"><a href="'.get_the_permalink($the_query->post->ID).'"><img src="'.$image_attributes[0].'"></a></span>
                       </div>
                    </div>
                    <div class="col-lg-4 col-md-4 col-sm-12">
                      <div class="agu_boxCol'.$popular.$breaknews.'">
                         <h2><a href="'.get_the_permalink($the_query->post->ID).'">'.$ttln.'</a></h2>
                         <p>'.$newscont.'</p>
                      </div>
                   </div>';
          }else{

              //if(strlen($the_query->post->post_title) > 37){
                //$ttln= substr($the_query->post->post_title, 0, 37) . '...';
              //}else{
                $ttln= $the_query->post->post_title;
              //}

              //if(strlen($the_query->post->post_excerpt) > 120){
                //$newscont= substr($the_query->post->post_excerpt, 0, 120) . '...';
              //}else{
                $newscont= $the_query->post->post_excerpt;
              //}
               $output.='<div class="col-lg-4 col-md-4 col-sm-12">
...
function fekete_shortcode( $atts, $content ) {
  ...
  if( $the_query->have_posts() ){
    ...
    while ( $the_query->have_posts() ) {
            ...
            if($i<=5){
              //if(strlen($the_query->post->post_title) > 62){
                //$ttln= substr($the_query->post->post_title, 0, 62) . '...';
              //}else{
                $ttln= $the_query->post->post_title;
              //}

              $first3[]='<p><i class="fa fa-plus-circle" aria-hidden="true"></i> <a href="'.get_the_permalink($the_query->post->ID).'">'.$ttln.'</a></p>';
                $last ='';
            }else{
              //if(strlen($the_query->post->post_title) > 40){
               //$ttln= substr($the_query->post->post_title, 0, 40) . '...';
              //}else{
                $ttln= $the_query->post->post_title;
              //}
              $last ='<span class="aug_img"><img src="'.$image_attributes[0].'"></span><label><a href="'.get_the_permalink($the_query->post->ID).'">'.$ttln.'</a></label>';
            }

        $i++;
    }

    $output.='<div class="col-sm-8 col-xs-12"><div class="agu_boxCol col_boxA7">'.join('',$first3).'</div></div>
<div class="col-sm-4 col-xs-12"><div class="agu_boxCol col_boxA8">'.$last.'</div></div>';
  ...
  if( $final_query->have_posts() ){
    while ( $final_query->have_posts() ) {
      ...
      if($j==1){
        ...
        $jarr[]='<div class="col-sm-4"><div class="lacus_sitDiv"><p><a href="'.get_the_permalink($final_query->post->ID).'">'.$final_query->post->post_title.'</a></p></div></div>';
      }
...
function section_sportnews_shortcode( $atts, $content ) {
  ...
  $output.='<div class="upr_prt">
      ...
      $i++;
            ...
            if($i==1){
              $sprt_thumb_img = wp_get_attachment_image_src( get_post_thumbnail_id($the_query->post->ID), 'sport1-thumb' );
        $output.='<div class="col-md-4 col-sm-12">
              <div class="colSBoxx colSBoxx1">
                 <h2><a href="'.get_the_permalink($the_query->post->ID).'">'.$the_query->post->post_title.'</a></h2>
                 <p>'.$the_query->post->post_excerpt.'</p>
              </div>
             </div><div class="col-md-5 col-sm-12">
              <div class="colSBoxx colSBoxx2">
                <span><a href="'.get_the_permalink($the_query->post->ID).'"><img src="'.$sprt_thumb_img[0].'"></a></span>
              </div>
            </div>';
      }elseif($i==3){
        $sprt_thumb_img = wp_get_attachment_image_src( get_post_thumbnail_id($the_query->post->ID), 'sport2-thumb' );
        $output.='<div class="col-md-3 col-sm-12">
              <div class="colSBoxx colSBoxx3">
                 <span><img src="'.$sprt_thumb_img[0].'"></span>
                 <label><a href="'.get_the_permalink($the_query->post->ID).'">'.$the_query->post->post_title.'</a></label>
              </div>
            </div>';
      }
  ...
  if( $the_query->have_posts() ){
    $output.='<div class="lower_prt">
          <div class="row">';
      while ( $the_query->have_posts() ) {
        $the_query->the_post();
          $output.='<div class="col-md-4 col-sm-12">
                <div class="spport_textB">
...
function section_stylenews_shortcode( $atts, $content ) {
  ...
  if( $the_query->have_posts() ){
    $output.='<div class="col-md-8 col-sm-12">';
        ...
        $output.='<div class="style_leftDiv"><img src="'.$artclebk_img[0].'" class="img-responsive">
                 <article>
                  <h2><a href="'.get_the_permalink($the_query->post->ID).'">'.$the_query->post->post_title.'</a></h2>
                  <label>'.$terms_string.' |  Szerző: <a href="'.home_url('/szerzo/').'?a='.$custom_author.'" class="athr_lnk">'.$custom_author.'</a></label>
                 </article>
              </div>';
         }
    $output.='</div>';
  ...
  if( $the_query->have_posts() ){
    $popular ='';
    $output.='<div class="col-md-4 col-sm-12">
          <div class="auto_float style_rightDiv">';
          while ( $the_query->have_posts() ) {
            $the_query->the_post();
            ...
            $output.='<p class="'.$popular.$breaknews.'"><a href="'.get_the_permalink($the_query->post->ID).'">'.$the_query->post->post_title.'</a></p>';
         }
    $output.='</div>
        </div>';
    }
        ...
        $output.='<div class="col-sm-4">
                <div class="style_Bx tyle_oBx1 styMainNewC">
                  ...
                  <div class="nues_content style_new_ContentBox'.$popular.$breaknews.'">
                   <p><a href="'.get_the_permalink($the_query->post->ID).'">'.$the_query->post->post_title.'</a></p>
                   <label class="f_wBig"><a href="'.home_url('/szerzo/').'?a='.$custom_author.'" class="athr_lnk">'.$custom_author.'</a><b></b></label>
                   <label>'.$dfrnc.'</label>
                  </div>
                </div>
               </div>';
...
add_filter( 'body_class', function( $classes ) {
	...
	if(isset($_COOKIE['dark_mode'])) { $dark_mode = true; }
...
function dm_switch_button_html(){
		...
		if(isset($_COOKIE['dark_mode'])) { $dark_mode = true; }
		$selected ='';
		if(isset($dark_mode)){$selected ='checked';	}
...
function breaking_bar_shortcode( $atts, $content ) {
	...
	if($show_breaking_bar){$runq = 1;} //for article pages	
	if(!$runq){return '';}//this is -ve condition
...
function section_euro2021( $atts, $content ) {
  ...
  .euro_container { /* background-color:#0f3246; */
  background-image:url('<?php echo get_template_directory_uri(); ?>/images/Eurocup_2020.png');
  background-repeat: no-repeat;
	background-position: right;
  padding-top: 23px;}
  .EURO2021-title{}
  .EURO2021-title img{  margin-left: 10%; width: 190px;}
	.bx-wrapper {background-color:transparent!important; -webkit-box-shadow: none !important;
    box-shadow: none !important;
    border: none!important;}
	.single-slide{background-color:#fff}
	.single-slide h3{line-height: 18px;
		padding: 12px;
		height: 150px;
	...
	.euro_bigpic{}
	.euro_smallpic{display:none;}
	.euro2021_tag_container{margin-top:12px}
	.euro2021_tag{border-bottom: 3px double #607d8b;   margin-left: 12px;   font-size: 1.082em !important; text-decoration:none; }

	 .euro2021_tag a {    font-size: 1.082em; }

	@media (max-width: 430px) {
		.euro_bigpic{display:none;}
		.euro_smallpic{display:block; padding-bottom: 20px; margin: 0 auto !important;}
		.single-slide{width:180px !important}
		.euro2021_tag{ font-weight: normal !important;}
		.euro_container { /* background-color:#0f3246; */
	  background-image:url('<?php echo get_template_directory_uri(); ?>/images/Eurocup_2020.png');
		background-repeat: no-repeat;
